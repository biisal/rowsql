...
export const TableForm = () => {
  ...
  const { data: serverDataTypes, isLoading: loadingDataTypes } = useQuery(
    newTableFormFiledsOptions(),
  );
  ...
  const dataTypes = useMemo(() => {
    ...
    return [
      ...(serverDataTypes.numericType || []),
      ...(serverDataTypes.stringType || []),
    ];
  }, [serverDataTypes]);
  ...
  async function onSubmit(values: FormValues) {
    const emptyColumns = values.inputs.filter(
      (input) => !input.columnName.trim(),
    );
